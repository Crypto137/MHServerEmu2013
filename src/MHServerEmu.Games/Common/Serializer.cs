...
namespace MHServerEmu.Games.Common
{
    ...
    public static class Serializer
    {
        ...
        public static bool Transfer(Archive archive, ref PrototypeId ioData)
        {
            ...
            {
                // V10_NOTE: Blueprint enums are not used for serialization in 1.10
                if (archive.IsPacking)
                {
                    ulong rawValue = (ulong)ioData;
                    success &= Transfer(archive, ref rawValue);
                }
                else
                {
                    ulong rawValue = 0;
                    success &= Transfer(archive, ref rawValue);
                    ioData = (PrototypeId)rawValue;
                }
            }
